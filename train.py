...
for num_epochs in range(50, 3000, 100):
    ...
    mse_train.append(mse_cal(train_loader, model))          
    mse_test.append(mse_cal(test_loader, model))            
    
    mae_train.append(mae_cal(train_loader, model))          
    mae_test.append(mae_cal(test_loader, model)) 
    
    print('epochs: %i, mse_train: %.3f, mse_test: %.3f' %(num_epochs, 
                                                          mse_cal(train_loader, model),
                                                          mse_cal(test_loader, model)))
